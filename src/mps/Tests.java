...
public class Tests extends JPanel implements ActionListener,MouseListener, MouseMotionListener, KeyListener{
    private int mySizeX;
    private int mySizeY;
    private int cellX;
    private int cellY;
    ...
    public Tests(int mySizeX, int mySizeY, String[][] Buttons){
        ...
        g.drawImage(image,nexx, nexy, interValX*2, interValY*2, this);
            ...
            for(int i = 0; i < characters.size();i++){
                ...
                for(int x = 0; x < me.RouteTaken.size()-1;x++){
                    ...
                    int firsty = me.RouteTaken.get(x).y*interValY+interValY/2;
                    int secondx = me.RouteTaken.get(x+1).x*interValX+interValX/2;
                    int secondy = me.RouteTaken.get(x+1).y*interValY+interValY/2;
                ...
                if(me.RouteTaken.size() > 0 && me.RouteTaken.get(me.RouteTaken.size()-1) != null){
                    ...
                    int firsty = me.RouteTaken.get(me.RouteTaken.size()-1).y*interValY+interValY/2;
                    int secondx = me.x+interValX/2;
                    int secondy = me.y+interValY/2;
                    g.drawLine(firstx, firsty, secondx, secondy);
                }
                /*
                g.setColor(Color.green);
            ...
            for(int i = 0; i < cellY; i++){
                for(int z = 0; z < cellX; z++){
                    if(Buttons[i][z].equals("S")){
                        ...
                        Paths.add(new PathV2(cellX,cellY,Butoners,interValX,interValY,hehe));
                    }
                }
            }
                searchT.start();
    ...
    public void actionPerformed(ActionEvent e){
        if(e.getSource() == t){
            ...
            for(int i = 0; i < Paths.size(); i++){
                if(Paths.get(i).tapos == true || Paths.get(i).prior == null)
                {
                    characters.add(new Minion(Paths.get(i).f,0,0));
    ...
    public void animateSearch(){
        ...
        if(!tapos && prior != null ){//&& Close.size() != countRegularNodes(Open)+1){
            ...
            characters.add(new Minion(pa,0,0));
        }
    }
    public void paintSearch(Graphics g){
        for(int x = 0; x < Paths.size();x++){
            ArrayList<Node> Close = Paths.get(x).Close;
            ArrayList<Node> Open = Paths.get(x).Open;
            Node prior = Paths.get(x).prior;
            g.setColor(Color.red);
            for(int i = 0; i < Close.size(); i++){
                if(Close.get(i).getParent() != null){
                    g.drawLine(Close.get(i).x*interValX+interValX/2, Close.get(i).y*interValY+interValY/2,Close.get(i).getParent().x*interValX+interValX/2, Close.get(i).getParent().y*interValY+interValY/2);
                }
                g.fillOval(Close.get(i).x*interValX+interValX/4, Close.get(i).y*interValY+interValY/4, interValX/2, interValY/2);
            }
            g.setColor(Color.blue);
            for(int i = 0; i < Open.size(); i++){
                if(Open.get(i).getParent() != null){
                    g.drawLine(Open.get(i).x*interValX+interValX/2, Open.get(i).y*interValY+interValY/2,Open.get(i).getParent().x*interValX+interValX/2, Open.get(i).getParent().y*interValY+interValY/2);
                }
                g.fillOval(Open.get(i).x*interValX+interValX/4, Open.get(i).y*interValY+interValY/4, interValX/2, interValY/2);
            }
            g.setColor(Color.cyan);
            if(prior!= null){
                if(prior.getParent() != null){
                    g.drawLine(prior.x*interValX+interValX/2, prior.y*interValY+interValY/2,prior.getParent().x*interValX+interValX/2, prior.getParent().y*interValY+interValY/2);
                }
                g.fillOval(prior.x*interValX+interValX/4, prior.y*interValY+interValY/4, interValX/2, interValY/2);
            }
            g.setColor(Color.black);
        }
    }
